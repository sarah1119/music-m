...
import {DELETEALL} from './types'
import {COLLECTALL}from './types'
...
const mutations = {
  [RENDER](state){
    ...
    // console.log(this)
    // this.$ajax.get('https://api.imjad.cn/cloudmusic/?type=url&id=9')
    //     .then(res => {
    //       context.commit('updateMessage', res.data.message);
    //       console.log(res)
    //       // resolve(res)
    //     })
    //     .catch(res => {
    ...
    //   $('button').click(function(){
        $.ajax({
          type:'GET',
          url:'http://localhost:3000/recommendLst',
          headers:{
            "Conten-Type":"http://localhost:3000/recommendLst"
          },
  ...
  [LAST](state){
    if (state.bIndex <=0) {
      state.bIndex = state.database.length-1
      state.audio.play();
      return;
    }
    state.bIndex -= 1
  ...
  [NEXT](state){
    if (state.bIndex >= state.database.length-1) {
      state.bIndex = 0
      state.audio.play()
      return;
    }
